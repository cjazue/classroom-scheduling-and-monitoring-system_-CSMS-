document.addEventListener('DOMContentLoaded', function () {
  ...
  function emitSelectedDate(selectedDate) {
    ...
    document.dispatchEvent(
      new CustomEvent('csms:date-selected', {
        detail: { date: selectedDate, iso },
      })
    );
  }
  ...
  function filterByDate(selectedDate) {
    // Kept for legacy pages that render static rows.
    ...
    document.querySelectorAll('.table-row').forEach(function (row) {
      ...
      if (!dateCell) return;
      row.style.display = dateCell.textContent.trim() === formatted ? 'grid' : 'none';
    });
  }
