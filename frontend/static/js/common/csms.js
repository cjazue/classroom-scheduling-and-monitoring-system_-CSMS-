...
(function () {
  ...
  const CSMS = {
    ...
    format: {
      displayName(full) {
        ...
        const tokens = knownSuffix ? parts.slice(0, -1) : parts.slice();
        const suffix = knownSuffix ? suffixRaw : '';

        // Find an explicit middle-initial token (e.g., "M" / "M.").
        // We intentionally do NOT derive an initial from "second name" tokens.
        let miIndex = -1;
        for (let i = 1; i <= tokens.length - 2; i++) {
          if (/^[A-Za-z]\\.?$/.test(tokens[i])) miIndex = i;
        }

        if (miIndex !== -1) {
          const first = tokens.slice(0, miIndex).join(' ');
          const last = tokens.slice(miIndex + 1).join(' ');
          const mi = String(tokens[miIndex] || '').replace(/[^A-Za-z]/g, '').slice(0, 1).toUpperCase();
          const base = first && last && mi ? `${first} ${mi}. ${last}` : tokens.join(' ');
          return suffix ? `${base} ${suffix}` : base;
        }

        // No explicit MI -> preserve name as-is (don’t invent one from second name).
  ...
  function wireLoginPage() {
    ...
    if (!p.endsWith('/loginpage.html')) return;
  ...
  function wireLogoHome() {
    ...
    logos.forEach((logo) => {
      // Make it feel clickable without requiring template edits.
      try {
        logo.style.cursor = 'pointer';
        logo.setAttribute('role', logo.getAttribute('role') || 'link');
        logo.setAttribute('tabindex', logo.getAttribute('tabindex') || '0');
        if (!logo.getAttribute('title')) logo.setAttribute('title', 'Go to dashboard');
      } catch {
        // ignore
      }
      ...
      async function goHome() {
        ...
        const user = await CSMS.auth.ensureUser();
        if (user && user.role) redirectTo(CSMS.paths.forRole(user.role));
        else redirectTo(CSMS.paths.landing);
      }

      logo.addEventListener('click', (e) => {
        // Don't hijack the hamburger toggle label inside the logo.
