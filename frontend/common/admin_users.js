...
(function () {
  ...
  async function loadStats() {
    ...
    try {
      const [roomsResp, pendingResp] = await Promise.all([
  ...
  function openImportUsers() {
    const input = $('userImportFile');
    if (!input) return;
    input.value = '';
    input.click();
  }

  function setImportResult(message) {
    const el = $('userImportResult');
    if (!el) return;
    if (!message) {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    window.openImportUsers = openImportUsers;
